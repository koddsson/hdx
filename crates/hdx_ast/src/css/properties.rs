...
pub struct CSSStyleProperty<'a> {
	name: Atom,
	value: CSSStyleValue<'a>,
	important: bool,
}

impl<'a> WriteCss<'a> for CSSStyleProperty<'a> {
	fn write_css<W: CssWriter>(&self, sink: &mut W) -> WriterResult {
...
fn get_global_value<'a>(parser: &mut Parser<'a>) -> Option<CSSStyleValue<'a>> {
	if parser.at(Kind::Ident) {
		match parser.cur_atom_lower().unwrap() {
			atom!("initial") => {
				parser.advance();
				Some(CSSStyleValue::Initial)
			}
			atom!("inherit") => {
				parser.advance();
				Some(CSSStyleValue::Inherit)
			}
			atom!("unset") => {
				parser.advance();
				Some(CSSStyleValue::Unset)
			}
			atom!("revert") => {
				parser.advance();
				Some(CSSStyleValue::Revert)
			}
			atom!("revert-layer") => {
				parser.advance();
				Some(CSSStyleValue::RevertLayer)
			}
			_ => None,
		}
	} else {
...
macro_rules! properties {
    ( $(
		...
		impl<'a> WriteCss<'a> for CSSStyleValue<'a> {
			fn write_css<W: CssWriter>(&self, sink: &mut W) -> WriterResult {
				match self {
		...
		impl<'a> Parse<'a> for CSSStyleProperty<'a> {
			fn parse(parser: &mut Parser<'a>) -> ParserResult<Spanned<Self>> {
				let span = parser.cur().span;
				let mut name = parser.expect_ident_cased()?;
				parser.expect(Kind::Colon)?;
				let value = if parser.cur().is_dashed_ident() {
					if let Some(val) = get_global_value(parser) {
						val
					} else {
						let value = Custom::parse(parser);
						if let Ok(value) = value {
							CSSStyleValue::Custom(parser.boxup(value))
						} else {
							let value = Unknown::parse(parser)?;
							parser.warn(diagnostics::UnknownDeclaration(value.span).into());
							CSSStyleValue::Unknown(parser.boxup(value))
						}
					}
				} else {
						...
						$(
							$atom => {
								...
								if let Some(val) = get_global_value(parser) {
									...
									let value = values::$name::parse(parser);
									if let Ok(value) = value {
										CSSStyleValue::$name(parser.boxup(value))
									} else {
										let value = Unknown::parse(parser)?;
										parser.warn(diagnostics::UnknownDeclaration(value.span).into());
										CSSStyleValue::Unknown(parser.boxup(value))
				...
				let important = if parser.cur().kind == Kind::Delim && parser.peek().kind == Kind::Ident && parser.peek().value.as_atom_lower().unwrap() == atom!("important") {
					parser.advance();
					parser.advance();
					true
				} else {
				...
				let end_span = if parser.cur().kind == Kind::Semicolon {
					let semi_span = parser.cur().span;
					parser.advance();
					span.until(semi_span)
				} else {
					span.until(parser.cur().span)
				};
				Ok(Self { name, value, important }.spanned(end_span))
			}
		}
    };
}
...
mod tests {
	...
	fn size_test() {
		use std::mem::size_of;
		assert_eq!(size_of::<CSSStyleProperty>(), 32);
		assert_eq!(size_of::<CSSStyleValue>(), 16);
	}
}
