...
use crate::{diagnostics, Parse, Parser, Result, Vec};

impl<'a> Parser<'a> {
	// https://drafts.csswg.org/css-syntax-3/#consume-a-qualified-rule
	pub fn parse_qualified_rule<
		T,
		Prelude: Parse<'a> + 'a,
		Rule: Parse<'a> + 'a,
		Decl: Parse<'a> + 'a,
	...
	pub fn parse_block<T, Rule: Parse<'a> + 'a, Decl: Parse<'a> + 'a, F>(
		&mut self,
		finalize: F,
	) -> Result<T>
	where
	...
	pub fn parse_declaration<T, Value: Debug + Parse<'a> + 'a, F>(
		&mut self,
		expected_name: Option<Atom>,
		finalize: F,
	) -> Result<T>
